      ...
      <CardContent>
        ...
        <Box sx={{ mb: 2 }}>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
            ...
            <Chip icon={<People />} label={`${totalCapacity} Total Capacity`} size='small' color={isValid ? 'success' : 'error'} />
          </Box>
          {totalCapacity > 0 && (
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {territories.map(territory => (
                <Box key={territory.id} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Chip label={`${territory.customerCount} customers`} size='small' color='primary' variant='outlined' />
                </Box>
              ))}
            </Box>
            <Typography variant='body2' color='text.secondary' sx={{ mt: 1 }}>
